...
# todo implement COCO, Pascal VOX, SUN3D, NYV in dataset class
# Keep labels
# class weights

# todo mapping dict between dataset
# todo inference on dataset for pseudo-labels
class RGBD_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir):
        ...
        if "SUNRGBD" in data_dir:
            self.used_dataset = 'sunrgbd'
            self.label_mapping = label_mapping

        self.data_samples = sorted(glob(self.data_dir + '/*'))
        # There is only one sample inside folder
        self.rgb_files = [glob(file + '/image/*')[0] for file in self.data_samples]
        self.depth_files = [glob(file + '/depth/*')[0] for file in self.data_samples]
        self.seg_files = [glob(file + '/seg.mat')[0] for file in self.data_samples]
    ...
    def __getitem__(self, idx):
        ...
        seg = loadmat(self.seg_files[idx])
...
# build preprocessor
trn_preprocessor = get_preprocessor(
        args,
...
val_preprocessor = get_preprocessor(
    args,
    dataset=data.datasets_valid[0],
    phase='test',
    multiscale_downscales=None
...
# get samples
data_dir = '/home/vacekpa2/data/SUNRGBD/realsense/all_samples'
exp_dir = data_dir + '/../example_exp/'

trn_dataset = RGBD_Dataset(data_dir)

for exp_folder in ['model', 'inference', 'segmentation']:
    os.makedirs(exp_dir + '/' + exp_folder, exist_ok=True)
